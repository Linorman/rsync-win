...
const LARGE_FILE_LEN: usize = 3 * 1024 * 1024;
const MAX_ALLOC: &str = "64K";

#[test]
fn daemon_push_whole_file_larger_than_max_alloc_streams_literals() {
    let temp = FixtureTempDir::new("rsync-win-large-file-push").unwrap();
    let server = LocalDaemonServer::start(&temp, false);
    let source_root = temp.path().join("source");
    fs::create_dir_all(&source_root).unwrap();
    let source_file = source_root.join("big.bin");
    write_pattern_file(&source_file, LARGE_FILE_LEN, 0x31).unwrap();
    ...
    let output = run_rsync_with_timeout([
        "-r",
        "--whole-file",
        &format!("--max-alloc={MAX_ALLOC}"),
        &source_arg,
        &dest_arg,
    ]);

    assert_command_success(&output, "large whole-file daemon push");
    assert_files_equal(&source_file, &server.module_root.join("big.bin"));
}

#[test]
fn daemon_pull_delta_larger_than_max_alloc_streams_basis_signatures() {
    let temp = FixtureTempDir::new("rsync-win-large-file-pull").unwrap();
    let server = LocalDaemonServer::start(&temp, true);
    let source_file = server.module_root.join("big.bin");
    write_pattern_file(&source_file, LARGE_FILE_LEN, 0x52).unwrap();

    let dest = temp.path().join("dest");
    fs::create_dir_all(&dest).unwrap();
    let basis_file = dest.join("big.bin");
    write_pattern_file(&basis_file, LARGE_FILE_LEN, 0x52).unwrap();
    patch_file_byte(&basis_file, LARGE_FILE_LEN / 2, 0x7f).unwrap();

    let source_arg = format!("{}/public/big.bin", server.url);
    let output = run_rsync_with_timeout([
        "-r",
        "--block-size=32768",
        &format!("--max-alloc={MAX_ALLOC}"),
    ...
    assert_command_success(&output, "large delta daemon pull");
    assert_files_equal(&source_file, &basis_file);
}
...
fn patch_file_byte(path: &Path, offset: usize, value: u8) -> io::Result<()> {
